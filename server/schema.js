...
const RootQuery = new GraphQLObjectType({
  ...
  fields: {
    movies: {
      //search for movies matching keywords
      ...
      resolve: async (parent, args) => {
        const res = await axios.get(
          omdbRootUrl + `s=${args.searchterm}&type=movie`
        )
        ...
        for (let movie of movies) {
          ...
          const res = await axios.get(
            omdbRootUrl + `i=${movie.imdbID}&plot=full`
          )
    ...
    movie: {
      //search for single movie. may not use for now
      type: movieType,
      args: {
        imdbID: {type: GraphQLString},
      },
